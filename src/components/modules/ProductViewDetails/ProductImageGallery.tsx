      ...
      <div className="grid grid-cols-1 gap-3 lg:gap-4 lg:h-[600px]">
        {product.images.slice(0, 3).map((img, index) => (
          <button
            ...
            className={`relative w-full aspect-[3/4] lg:flex-1 overflow-hidden border-2 transition-all duration-300 ${activeImage === index ? '' : ''}`}
          >
            <Image
              fill
      ...
      >
        <AnimatePresence mode="wait">
          ...
          >
            <Image
              fill
              src={`${process.env.NEXT_PUBLIC_API_URL}${images[activeImage]?.url}`}
